...
def efficientnet_yolo_lite(inputs, model_name, num_anchors, **kwargs):
    ...
    return tf.keras.models.Model(efficientnet.inputs, [y1, y2, y3])
...
def SEBlock_ind(input_filters, filters):
    num_reduced_filters = input_filters * 0.25
...
def MBConvBlock_ind(inputs, border_filters):

    has_se = True
    expand_ratio = 6
    input_filters = border_filters
    output_filters = border_filters
    kernel_size = 5
    strides = [1, 1]
    ...
    x = tf.keras.layers.DepthwiseConv2D(
        ...
        padding='same',
        use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(momentum=batch_norm_momentum,
                                           epsilon=batch_norm_epsilon)(x)
    x = Swish()(x)

    if has_se:
        x = SEBlock_ind(input_filters, filters)(x)
...
def _inverted_res_block(inputs, border_filters):
    ...
    in_channels = border_filters
    pointwise_filters = border_filters
...
if True:
    ...
    # K.set_session(sess)
    border_filters = 80
    inputs = tf.placeholder('float32', shape=(1,28,28,border_filters))
    inputs = tf.keras.layers.Input(tensor=inputs)
    base_model = MBConvBlock_ind(inputs, border_filters)
