...
def get_random_data(image,
    ...
    if(zoom_in):
        input_shape=tf.keras.backend.get_value((416, 416))
    ...
    #                         [tf.cast(w/dum_ratio, tf.int32),
    #                          tf.cast(h/dum_ratio, tf.int32)])
    #
    # image = tf.image.resize(image,
    #                         [tf.cast(w, tf.int32),
    #                          tf.cast(h, tf.int32)])
    ...
    xmaxs = tf.clip_by_value(xmaxs,
                            clip_value_min=0,
                            clip_value_max=tf.cast(input_shape[1] - 1,
                                                   tf.float32))
    xmins = tf.clip_by_value(xmins,
                            clip_value_min=0,
                            clip_value_max=tf.cast(input_shape[1] - 1,
                                                   tf.float32))
    ymaxs = tf.clip_by_value(ymaxs,
                            clip_value_min=0,
                            clip_value_max=tf.cast(input_shape[0] - 1,
                                                   tf.float32))
    ymins = tf.clip_by_value(ymins,
                            clip_value_min=0,
                            clip_value_max=tf.cast(input_shape[0] - 1,
                                                   tf.float32))
...
def preprocess_true_boxes(true_boxes, input_shape, anchors, num_classes, num_scales):
    ...
    anchor_mask = anchor_mask[-1*num_layers:]
    ...
    grid_steps = [32, 16, 8]
    grid_shapes = [
        np.round(input_shape / grid_steps[l]).astype(np.int32)
...
class ModelFactory(object):
    ...
    def build(self, model_builder, *args, **kwargs):
